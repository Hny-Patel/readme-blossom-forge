-- ============================================================
-- Admin System Migration v2 (idempotent — safe to re-run)
-- Drop + recreate all policies and triggers cleanly
-- ============================================================
...
DROP POLICY IF EXISTS "Users can read plans" ON public.plans;
...
(
  'free', 'Free', 'Get started with basic ledger features',
  0, 0, 0,
  '{"max_businesses":1,"max_accounts":10,"max_transactions_per_month":50,"has_analytics":false,"has_pdf_reports":false,"has_cashbook":false,"has_expenses":false,"has_data_export":false}'::jsonb
),
(
  'starter', 'Starter', 'For small businesses growing fast',
  299, 2990, 1,
  '{"max_businesses":2,"max_accounts":100,"max_transactions_per_month":500,"has_analytics":true,"has_pdf_reports":true,"has_cashbook":true,"has_expenses":false,"has_data_export":false}'::jsonb
),
(
  'pro', 'Pro', 'Full-featured for serious businesses',
  699, 6990, 2,
  '{"max_businesses":5,"max_accounts":null,"max_transactions_per_month":null,"has_analytics":true,"has_pdf_reports":true,"has_cashbook":true,"has_expenses":true,"has_data_export":true}'::jsonb
),
(
  'enterprise', 'Enterprise', 'Unlimited everything for large operations',
  1499, 14990, 3,
  '{"max_businesses":null,"max_accounts":null,"max_transactions_per_month":null,"has_analytics":true,"has_pdf_reports":true,"has_cashbook":true,"has_expenses":true,"has_data_export":true}'::jsonb
)
...
DROP POLICY IF EXISTS "Admins can manage login_restrictions" ON public.login_restrictions;
CREATE POLICY "Admins can manage login_restrictions"
  ON public.login_restrictions FOR ALL
  USING (EXISTS (SELECT 1 FROM public.admin_users WHERE user_id = auth.uid()));

DROP POLICY IF EXISTS "Users can read own restrictions" ON public.login_restrictions;
...
DROP POLICY IF EXISTS "Admins can read all audit_log" ON public.audit_log;
CREATE POLICY "Admins can read all audit_log"
  ON public.audit_log FOR SELECT
  USING (EXISTS (SELECT 1 FROM public.admin_users WHERE user_id = auth.uid()));

-- ── 8. Auto-subscription trigger ────────────────────────────
